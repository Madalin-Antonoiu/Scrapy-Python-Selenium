...
async function hiddenWoId(safe4){
    ...
    if(safe4.AssignTechnicianPopup.style.visibility == "visible" && currentSelectedGroupText == 'Service Desk' ){
        ...
        // if formHiddenInputTicketId = passed in ticket
        return {'message' : 'Assign Technician is OPEN, Service Desk currently selected',
                'formHiddenWoID': formHiddenInputTicketId,
                'ticketId': safe4.ticketId,
                'currentTechnician': safe4.currentTechnician,
                'currentGroup': safe4.group_div.children[0].children[0].textContent
                }
    }
...
async function assign(ticket){
    ...
    let assign = await document.queryXPath("//label[@class='formsubmitbtn-label-ui1']")[0].children[0];
    //yet another safety measure

    if(ticket.ticketId == ticket.formHiddenWoID && assign !== null){


        if(ticket.currentGroup == "Service Desk"){

            //assign it
            // update storage
            return {'Last step':'Here i assign it',
                    'formHiddenWoID': ticket.formHiddenWoID,
    ...
    throw new Error('Failed to assign - check me.')
}

//Main function
async function main(row_to_try) {
    //try catch is for calls only, not for methods above
    try{

        //enforce a refresh nd reinject of all code except the call main function (from selenium)
        const safe = await retry (inGroupUnassigned(), 10, 500); // get element will return the element from within, dont put try 0 times lol
        ...
        const safe7 = await retry (hiddenWoId(safe4), 5, 500);
        console.log(await safe7);

        const safe8 = await retry (assign(safe7), 5, 500);
        console.log(await safe8);

        storage = await safe8;
        executed += `[${storage.ticketId} - ${storage.currentTechnician}] : Assigned to ${storage.currentGroup}` + '\n' ;
        executed_last = `[${storage.ticketId} - ${storage.currentTechnician}] : Assigned to ${storage.currentGroup}` ;

        console.log(executed)
        console.log(` %c ${executed_last} `, 'background: #DC5B4A; color: #FFFFFF');

        //console.log(` %c ${storage.ticketId} `, 'background: #222; color: #bada55' );
        //could skip execute thing and console.log storage.ticketId with custom css
...
var executed = ["Records:\n"]; // this is what holds what i executed
var executed_last = "Last Actions Taken:\n";
//var row_to_try = 6;
